...
| My page | `GET /api/v1/users/me/summary` plus existing user APIs | covered | `profile`, `stats.monthlySaving`, `favoriteCount`, `activeAlertCount`, `recentOrderCount`, `preferences`, `allergyCodes`, `personalizedIngredients`, `menuRows` | real order/saving calculation | Demo user has preference, allergies, favorite, alert | Visible: `/api/v1/users/me/summary`; example `FrontendMyPageSummaryResponse` | `monthlySaving` and `recentOrderCount` are zero until order/settlement domains exist. |
...
- My page: demo user preference, allergies, favorite, price alert.
