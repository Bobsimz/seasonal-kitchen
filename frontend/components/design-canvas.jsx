...
if (typeof document !== "undefined" && !document.getElementById("dc-styles")) {
  ...
  s.textContent = [
    ...
    // PDF capture mode — hides chrome while downloadBoardsPDF() rasterizes
    // each section with html2canvas. Canvas background tone is preserved.
    "body.dc-pdf-capturing .twk-panel,",
    "body.dc-pdf-capturing .dc-header,",
    "body.dc-pdf-capturing [data-omelette-chrome]{display:none !important}",
  ].join("\n");
...
// ─────────────────────────────────────────────────────────────
// DCHTMLButton — floating "HTML 다운로드" trigger. Reuses dcExport's
// HTML pipeline (computed styles + fonts + images all inlined) so the
// downloaded file is a standalone snapshot. Open in browser → Cmd+P →
// Save as PDF for pixel-perfect output.
// ─────────────────────────────────────────────────────────────
function DCHTMLButton() {
  const [busy, setBusy] = React.useState(false);
  const onClick = async () => {
    if (busy) return;
    setBusy(true);
    try {
      await downloadBoardsHTML();
    } finally {
      setBusy(false);
    }
  };
  return (
    ...
    >
      {busy ? "HTML 생성 중…" : "HTML 다운로드"}
    </button>
  );
}

async function downloadBoardsHTML() {
  const world = document.querySelector("[data-dc-world]");
  if (!world) return;

  const saved = {
    worldTransform: world.style.transform,
    worldZoom: world.style.getPropertyValue("--dc-inv-zoom"),
  };
  world.style.transform = "none";
  world.style.setProperty("--dc-inv-zoom", "1");
  document.body.classList.add("dc-pdf-capturing");
  ...
  try {
    ...
    world.style.transform = saved.worldTransform || "";
    if (saved.worldZoom)
      world.style.setProperty("--dc-inv-zoom", saved.worldZoom);
    document.body.classList.remove("dc-pdf-capturing");
  }
}
        ...
        >
          {order.map((k) => (
            <DCArtboardFrame
              ...
              onReorder={(next) =>
                ctx && ctx.patchSection(sid, { order: next })
              }
              onDelete={() =>
                ctx &&
                ctx.patchSection(sid, (x) => ({
...
async function dcExport(node, w, h, name, kind) {
  ...
  const cloneStyled = (src) => {
    ...
    if (src.nodeType === 1) {
      ...
      // `line-height: normal` resolves against the font's metrics, which the
      // SVG foreignObject often picks a different fallback for — that
      // mismatch shows up as extra leading between lines. Pin it to the
      // pixel value the live DOM is rendering so the export matches.
      let extra = "";
      if (cs.lineHeight === "normal") {
        const fs = parseFloat(cs.fontSize) || 0;
        if (fs > 0) extra += "line-height:" + fs * 1.2 + "px;";
      }
      dst.setAttribute(
        "style",
        txt + extra + "animation:none;transition:none;",
      );
      if (src.tagName === "CANVAS")
        try {
          const im = document.createElement("img");
          im.src = src.toDataURL();
          im.setAttribute("style", txt + extra);
          return im;
        } catch {}
    }
...
function DCArtboardFrame({
  ...
  const doExport = (kind) => {
    ...
    // Use the card's actual rendered size — stretch-mode screens grow past
    // the declared PHONE_H, and capping at `height` clips the bottom.
    const w = cardRef.current.offsetWidth || width;
    const h =
      cardRef.current.scrollHeight || cardRef.current.offsetHeight || height;
    dcExport(cardRef.current, w, h, name, kind).catch((e) =>
