...
// showCart=true(기본): 담기(+) 버튼을 <Link> 밖에 둬서 네비게이션과 충돌하지 않게 한다.
// showCart=false: 담기 버튼 없이 카드 전체가 상세로 이동(식재료 상세의 "제품" 섹션 등).
export function ProductCard({ product: p, href, className, showCart = true }) {
  const soldOut = p.stockStatus === 'SOLD_OUT';
  const sub = [p.region, p.producerName].filter(Boolean).join(' ');
  const target = href || `/producers/${p.producerId}`;
  ...
  // 담기 버튼 없는 카드 — 가격까지 포함해 카드 전체가 하나의 링크(상세로 이동).
  if (!showCart) {
    return (
      <Card className={cn('flex flex-col overflow-hidden', className)}>
        <Link href={target} className="tap flex flex-1 flex-col">
          {media}
          {info}
          <div className="mt-auto px-3 pb-3 pt-2">{price}</div>
        </Link>
      </Card>
    );
  }
  ...
  return (
    <Card className={cn('flex flex-col overflow-hidden', className)}>
      <Link href={target} className="tap block">
        {media}
        {info}
      </Link>
